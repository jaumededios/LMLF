# Stage 1 first finite Chapter 4 packet companion contract — draft 04

## Status and authority boundary

This is a bounded natural-language proposal for the smallest executable source-inventory and classification contract that could enable one finite Chapter 4 implementation packet. It succeeds, and does not alter, `stage1-companion-schema-design-draft-03.md`.
...
No approval of draft 03 transfers to these changed bytes; any prior review remains historical evidence only.
...
Every effective `ordinary_exact_or_structural` classification contains, or exactly binds within the
same classification artifact, a nonempty frozen occurrence-specific rationale stating both why the
assertion is not asymptotic or approximative and why the finite-producer plus faithful-recovery pair
is inapplicable. The rationale binds the exact atom, snapshot, schema set, and classification view;
the containing classification artifact/view retains exact ID and exact-byte digest bindings. Inline
rationale is frozen by that artifact digest, so no additional artifact family is required.

E0 checks only nonemptiness, binding equality, and exact IDs/digests. Manual source/classification
reviewers and root authority establish the rationale's mathematical truth; the validator does not
infer or approve that truth.
...
3. **`exact-signature-binding`:** each role binds the exact declaration in an independently reviewed,
   frozen exact-signature packet and references the exact selected pre-signature gate bundle below.
   It may be externally selected only when every required gate in that bundle passes or has the
   permitted reviewed `not_required` state. Quantitative roles bind two distinct declarations and cards.

That pre-signature gate bundle binds the complete natural-language proof and all applicable required
multiple fresh proof/quantitative and architecture/structural-circularity reviews. A protocol-permitted
inapplicable gate instead has an exact-bound reviewed `not_required` record with its applicability
reason; absence never means inapplicability. Bundle rows bind exact artifact IDs/digests and relevant
reviewer IDs and controlled-vocabulary perspectives.
...
These checks do not implement a review-quorum or effective-authority fold. In particular, E0 can
confirm that supplied synthetic gate records are typed and digest-bound, but cannot establish actual
pass, freshness, reviewer independence, perspective sufficiency, applicability, or quorum.
...
- the positive ordinary fixture carries a nonempty frozen occurrence-specific rationale with exact
  atom/snapshot/schema-set/classification-view and containing-artifact ID/digest bindings; blank or
  missing rationale, wrong atom, wrong view, or digest mismatch fails;
- a syntactically valid ordinary token on the synthetic asymptotic atom fails only because it
  mismatches the exact supplied externally reviewed synthetic source-semantics binding, never by NLP inference;
...
- a positive exact-signature fixture binds its exact selected pre-signature gate bundle, including an
  explicitly justified reviewed `not_required` gate where permitted; missing required gate or
  perspective, or silently treating an absent structural-circularity gate as inapplicable, fails;
...
Cards and dependency closure follow as separately reviewable gates. Before exact-signature design or
freezing, complete natural-language proofs and all applicable required multiple fresh
proof/quantitative and architecture/structural-circularity reviews must pass; protocol-permitted
inapplicability must be explicitly reviewed and justified. Exact-signature review, `lean_ready`,
implementation, proof review, and final exact-head integration then remain separate gates.
...
These are author-proposed dispositions preserved from draft 03. They are not external resolutions,
do not make any prior verdict terminal, and transfer no approval or quorum to draft 04.
...
## Appendix C — disposition of draft-03 referee D findings

These are author-proposed dispositions only and require fresh exact-artifact review of draft 04.

1. Referee D G1 is accepted in **Orthogonal DLMF classification axis** and focused E0 tests: every
   effective ordinary class has the exact-bound nonempty frozen occurrence rationale, while E0 checks
   only its structure and manual authority retains mathematical judgment.
2. Referee D G2 is accepted in **Role cardinalities and three validation levels**, focused E0 tests,
   and **First real packet after E0**: exact-signature binding names the complete selected
   pre-signature gate bundle without automating pass, independence, applicability, or quorum.
