# Proposed Stage 1: DLMF through Chapter 10 — revision 3
...
**Decision proposed here:** `DD-021`, revision 2
...
This is the complete revision-3 successor proposal to
[`stage_1_dlmf-R2.md`](stage_1_dlmf-R2.md). Revisions 1 and 2 remain immutable
historical review inputs. R2's exact bytes received a `request_changes`
verdict identifying planning-closure defects; that verdict is not a claim of a
current production emergency. Neither predecessor has operative authority.
This document authorizes no source census, schema migration, theorem card,
Lean implementation, coverage claim, or Verso authority.

## DD-021 revision 2 — proposed Stage 1 programme precedence
...
This **operative plan is still pending**. DD-021 revision 2 takes effect only
after both of the following exist for the exact same five-file successor bytes
(`stage_1_dlmf-R3.md` and the successor notices in `README.md`,
`roadmap.md`, `design_decisions.md`, and `source_manifest.md`):
...
hold, the existing frozen control-plane text remains operative and this R3 is
...
Each finite reviewed source-audit manifest must enumerate every canonical URL
in its explicit closed batch. The global `B_all` manifest must enumerate the
full canonical Chapter 4--10 URL census. Each actual retrieval is bound to its
retrieval date, content digest, retrieval method, and observed version evidence.
A root version label is not a per-page digest.
...
complete payload of every canonical page. Each leaf receives a reviewed
...
every canonical page item has one source-owning leaf. This does **not** require
...
`source_appearance_id` binds its canonical URL, leaf, locator/span evidence,
representation digests, and appearance ordinal. Two appearances of the same
formula or prose claim have different appearance identities and therefore
remain distinct occurrences. Within one appearance, each semantic atom has one
ordinal and one active occurrence ID; two IDs for the same ownership key are
forbidden. Human-readable locators may change without changing the key.
...
source reviewer checks every source partition, atomicization, fidelity-role,
exclusion, and provisional-resolution decision against the exact snapshot.

### Append-only resolution and lineage
...
historical row. Corrections and resolution use append-only decision and lineage
records. An active atom is a real atomic record that has not been superseded and
whose full lineage and effective resolution have approving exact-bound reviews.

A split must name a nonempty successor set and prove that the successors'
semantic claim ownership is pairwise nonduplicative and exactly exhausts the
predecessor's semantic claim. A merge must prove that its successor owns
exactly the union of its named predecessors, without loss or duplication.
Shared context references remain permitted and do not violate this invariant.
Superseded atoms retain their immutable IDs and evidence but are inactive and
cannot be counted alongside active successors. A zero-successor transition is
forbidden unless an exact-bound reviewed snapshot-removal or exclusion decision
accounts for the whole former payload; it can never silently resolve a
substantive claim. No missing, overlapping, or unjustified empty successor set
is valid.

An underspecified claim remains `provisional_unresolved_claim` and blocks
completion. Review may resolve it by append-only creation of one or more real
atomic target records, or by an exact-bound decision showing that the complete
payload is reference-only/nonmathematical. The provisional ID never enters the
target set and is never relabeled. Thus resolving fixture `FX-P1` by creating
`FX-O5` can add only `FX-O5`; the pair can never both count.
...
snapshot, canonical pages/source-owning leaves, shared contexts, and all
required hierarchy needed to interpret them. Its membership is explicit:
...
leaf it names is exact-bound. All of `B`'s source-partition, atomicization,
fidelity-role, exclusion, and effective-resolution reviews must be present and
approving, with no unresolved or invalid lineage inside the batch. Missing,
pending, `changes_requested`, or failed review makes it false.
...
Target(S,B) = {
  o.occurrence_id |
    ...
    and 4 <= o.chapter <= 10
    and o is active and successfully resolved
...
`Target(S,B)` is a set of immutable active atomic IDs, not a count of pages,
...
remain historical but cannot be counted with active successors. One occurrence
...
A separately reviewed global closure audit batch `B_all` must prove that its
canonical URL/leaf membership exhausts Chapters 4--10 and that the source union
has neither gaps nor duplicate ownership. Only when
`PartitionReady(S,B_all)` succeeds is the whole-programme value defined:

```text
Target(S) = Target(S,B_all)
```

Before that global gate, `Target(S)` is undefined even when one or more local
`Target(S,B)` values exist. Reviewed finite audit batches may nevertheless
...
Every member of any defined `Target(S,B)` has exactly one active immutable
source-semantics classification:
...
inapplicable. Missing, duplicate-active, semantically wrong, pending, failed,
...
| global audit closure | the separately reviewed finite audit batch `B_all` proves that its canonical URL/leaf membership exhausts Chapters 4--10, and `PartitionReady(S,B_all)` holds: the complete hierarchy/leaf inventory and semantic atomization are exact-bound, exhaustive, nonduplicative, and all partition, atomicization, role, exclusion, and resolution/lineage reviews approve |
...
| target denominator | `Target(S) = Target(S,B_all)` is defined by §4 from the active successfully resolved atoms; every member has confirmed applicable notation/entity links and complete card/declaration coverage |
...
| obligation classification | every member of `Target(S)` has exactly one active immutable §5 classification with approving exact-bound external source review; every ordinary target has its reviewed frozen inapplicability reason |
...
| a mathematical assertion in a batch leaf has no atom | that semantic source partition is not exhaustive and `Target(S,B)` is undefined; if it occurs in `B_all`, `Target(S)` is also undefined |
...
| a split/merge has a semantic gap, duplicate/overlapping claim ownership, or an unjustified zero-successor result | active lineage does not exactly preserve semantic payload |
...
| a target has no obligation classification, or has two active classifications | classification is not total and single-valued |
...
| a finite reviewed audit batch exists but its local count is reported as the whole Stage 1 denominator or percentage | `Target(S,B)` does not define `Target(S)` without reviewed `B_all` |
...
Let `FX-B` be a finite audit batch containing these six leaves. The five
active atoms have distinct canonical keys
...
```text
Target(FX-S,FX-B) = {FX-O1, FX-O2, FX-O3, FX-O4, FX-O5}
```
...
five members and equals `Target(FX-S,FX-B)`. This shows that many-to-many
associations do not inflate source counts. Before `FX-P1` is successfully
resolved and the batch partition gate passes, `Target(FX-S,FX-B)` is
undefined, not an empty or partial set. The whole-programme `Target(FX-S)`
also remains undefined unless a separate fixture `FX-B-all` receives reviewed
global closure; this small example makes no such claim.
...
1. Create the reviewed multi-track schema migration and DLMF source-manifest
   ...
   may vary, but it may not weaken §§4--5's partition, ownership, lineage, or
   total-classification semantics.
2. Create a finite, explicit Chapter 4 source-audit batch `B` and retrieve
   its URL/content-unit census, recording actual
   per-page dates, digests, and version evidence. Build the
   container/source-leaf inventory and perform forward and reverse passes.
   Obtain independent exact-bound review of source partitions, semantic
   atomization, fidelity roles, exclusions, and resolutions. Publish counts
   only as `B`-scoped values after `PartitionReady(S,B)`; do not report a
   Stage 1 percentage.
...
source-appearance, context, occurrence, obligation, notation, entity, card,
dependency, decision/lineage, external-review, audit-batch, and Lean execution
...
otherwise undefined denominator, or turn a reference lead into coverage.
...
external gate, closed/complete DLMF execution manifest, Lean implementation, or
coverage percentage is claimed.

DD-021 revision 2 and this five-file successor remain pending until the exact
