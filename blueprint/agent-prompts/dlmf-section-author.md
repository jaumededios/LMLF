...
## Object-first API rule

Treat reader complaints about one statement as evidence about the surrounding
API. Before writing a large proposition, identify the mathematical object that
the source is manipulating and give that object the smallest honest reusable
interface.

- Put persistent invariants in a structure or subtype instead of repeating them
  as hypotheses. Examples include terminated coefficient streams, admissible
  paths, ordered zeros, normalized recurrence solutions, and remainder data.
- Separate syntax, evaluation, convergence, and error control. A finite
  continued fraction is not its convergence theorem; a formal expansion is not
  analytic equality; an exact identity is not a quantitative remainder bound.
- Prefer a canonical map with reconstruction and uniqueness theorems over an
  existential followed by `Classical.choose`.
- Expose a finite-prefix or executable theorem when source data are generated by
  an algorithm.
- Give secondary controls sensible defaults, such as an optional tail offset.
- For path integrals, quantify a typed contour (and any required branch lift)
  and use its integral. Do not inline pages of parameterization and limit
  plumbing into every special-function theorem.
- Reject arbitrary-function placeholders and tautological remainder
  decompositions. Named objects must be tied to the DLMF function or data they
  represent.

If the reusable interface belongs in Chapters 1–3, record it as shared
vocabulary and surface it there; do not leave the reader to reverse-engineer a
one-page local encoding.
