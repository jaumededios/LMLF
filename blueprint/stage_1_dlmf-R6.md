# Proposed Stage 1: DLMF through Chapter 10 — revision 6
...
**Decision proposed here:** `DD-021`, revision 5
...
This is the complete revision-6 successor proposal to
[`blueprint/stage_1_dlmf-R5.md`](stage_1_dlmf-R5.md). Revisions 1--5 remain
immutable historical review inputs. R5's exact bytes received two
`request_changes` reviews identifying bounded specification defects; neither
verdict is a claim of a current production emergency. Root adjudicates referee
I's G1 as P1 for this proposed plan, rather than a P0 production trust incident.
...
## DD-021 revision 5 — proposed Stage 1 programme and architecture amendment
...
This **operative plan is still pending**. DD-021 revision 5 takes effect only
after both of the following exist for the exact same seven-file successor bytes,
...
- `blueprint/dependency_graph.md`; and
- `blueprint/stage_1_dlmf-R6.md`.
...
2. a root external adjudication recording acceptance, also bound to that exact
   commit, the seven artifact digests, and the two review identities.

Candidate-owned prose cannot satisfy either condition. Until both conditions
hold, the existing frozen control-plane text remains operative and this R6 is
...
The only additional status repair is the bounded correction of repeated
`IMP-001` registration/lifecycle descriptions and dependent next-step labels
within `blueprint/dependency_graph.md`. The seven-file successor replaces those
stale descriptions with the frozen registered R7/`IMPROPER-0` specification
status already established by the canonical baseline inventory. This changes
no mathematical artifact, implementation state, or review credit.

No subject outside those two bounded areas and that one bounded status repair
is superseded or narrowed. Legacy
...
Decision-view replay and rerunnable source evidence are separate claims. Every
new or renewed source audit must bind an immutable, access-controlled
exact-payload evidence object, or an authorized archive whose contract
guarantees the exact bytes. Its record names the payload digest, retrieval
metadata, continuing availability, access conditions, and rights/provenance.
The code repository retains only locators, digests, metadata, short
indispensable fragments, and independent paraphrases; it does not hold bulk
DLMF or book copies, and this proposal authorizes no download. If rights or
availability prevent evidence retention, the audit is explicitly
`non_reproducible` or `expired`: its immutable historical decision record and
denominator can still be replayed as records, but a digest alone cannot renew
source approval or support a claim of current reproducible completion.
...
| 8 | [Incomplete Gamma and Related Functions](https://dlmf.nist.gov/8) | Audit §§8.1--8.27; treat §8.28 analogously. Leads include `gamma(a,z)`, `Gamma(a,z)`, `gamma*(a,z)`, `P`, `Q`, their [generalizations](https://dlmf.nist.gov/8.16), incomplete beta `B_x`,`I_x`, generalized exponential `E_p`, generalized sine/cosine integrals; the terminant, incomplete-zeta, and Debye names in [§8.22](https://dlmf.nist.gov/8.22); and Prym's alternative notations in [§8.1](https://dlmf.nist.gov/8.1). |
...
| 10 | [Bessel Functions](https://dlmf.nist.gov/10) | Audit §§10.1--10.76; treat §10.77 analogously. Leads include `J_nu`,`Y_nu`, Hankel `H_nu^(1,2)`, modified `I_nu`,`K_nu`, imaginary-order forms and order derivatives; §10.46 as distinguished below; spherical and modified spherical Bessel functions; Kelvin `ber`,`bei`,`ker`,`kei` and modulus/phase auxiliaries; Rayleigh's function in [§10.21(xiii)](https://dlmf.nist.gov/10.21#xiii); and the Bickley function in [§10.43(iii)](https://dlmf.nist.gov/10.43#iii). |
...
These leads remain explicitly nonexhaustive until a reviewed discovery pass
closes the chapter landing pages, special-notation pages, every `Defines` and
symbol annotation, and reverse passes through the official index and notation
list. That lead-closure checklist is distinct from atomic source partition and
cannot establish `Target(S,B)` or any coverage percentage. A lead may resolve
to identity/context rather than a theorem, but it may not simply disappear.
...
The selected current page's attached mathematical annotations and editorial
notes are source payload too. `Defines` and symbol annotations normally supply
entity/notation identity or shared context; they mint a separate target atom
only when they independently assert new mathematics. A current corrective
mathematical note attached to the selected page is included. Superseded errata
formulas and change-history entries remain provenance for historical states,
not competing current truths. Annotated and unannotated renderings are
representations of the same payload and must be reconciled without duplication.
...
Source identity and atomization corrections do not repair mathematical-object
identity. Three further append-only typed relations correct (a) notation
identity, (b) entity identity, and (c) occurrence-to-notation/entity
association. Each split or coalescence allocates fresh successor IDs and names
all predecessors; no relation relabels, recycles, or rebinds an old key. A
value-preserving notation alias is recorded separately from a value-changing
normalization bridge, whose conversion theorem, domain, branches, and argument
or parameter transformation must be explicit. Equal spelling does not prove
entity identity, and an identification theorem cannot be used as source-target
credit.

An audit revision pins the exact notation/entity/association decision view it
used, separately from its source-partition view; every dependent Lean execution
manifest pins the same view by ID and digest. Such a view may be incomplete
during source census. An unresolved object identity does not make an otherwise
reviewed source atom disappear and does not by itself invalidate the source
partition or denominator. It does block named-card execution authorization and
completion for that atom until its exact branch/entity view is reviewed.
Acceptance of a repair suspends the affected identification, card,
`lean_ready`, implementation, coverage, and recovery authority until an exact-
bound successor review succeeds. It does not revoke Lean kernel validity,
change the source denominator without a separate source correction, or suspend
unrelated batches and declarations.
...
obligation-classification decision view used for its denominator; it never
performs a mutable “latest record” lookup. It separately records the
notation/entity/association view used by any dependent work, including an
explicit unresolved state. Its membership is explicit:
...
fidelity-role, exclusion, effective-resolution/lineage, and source-semantics
classification reviews must be present and approving for the pinned source
view, with no unresolved source claim or invalid lineage inside the batch.
Missing, pending, `changes_requested`, or failed source/classification review
makes it false. Entity/notation resolution is a later execution/completion
gate; its pending state cannot turn a real source denominator into an empty or
undefined value once the source partition and classification are ready.
...
only when their pinned source and obligation-classification decision views
agree on the overlap; incompatible views require explicit reconciliation and
fresh review, while execution also requires compatible notation/entity/
association views.
...
revision and its denominator remain replayable and unchanged as decision
records. Rerunning its source audit additionally requires the retained exact
payload evidence specified in §2.
...
The same current-authority rule governs every local batch used to authorize a
Lean execution manifest. Its external root selection record binds `S`, the
local batch revision/digest, all pinned source, classification, and
notation/entity/association views, applicable approving review identities, and
dependent authorizations. Global closure additionally requires the two fresh
independent reviewers above. Every selected authority view is an explicit fold
of the bound external records: it names supersession, suspension, and
withdrawal records and may not omit accepted adverse evidence or substitute a
mutable “latest timestamp.”

Acceptance of material counterevidence that undermines a selected local or
global audit must, in the same exact-bound root external record, suspend or
withdraw that authority and its affected execution gates and current coverage.
The root may also suspend on recorded, justified material doubt. A positive
review quorum is not required for conservative suspension; an unreviewed
allegation alone is not accepted counterevidence and makes no binding change.
Suspension makes the affected current authority and its dependent completion
claims false/pending until an exact reviewed successor is selected, with no
silent fallback to an older selection. Global suspension makes current
`Target(S)` undefined; local suspension withdraws that batch's current
authority without deleting its historical set. Unaffected local batches may continue.
Every immutable historical local or global `Target(S,B_old)` remains replayable
as a historical set, but a suspended selection may no longer be presented as
valid current coverage. Selection and withdrawal cannot change the user-
authorized Chapters 4--10 material boundary. A material scope amendment again
requires two fresh independent blueprint approvals and root external
adjudication bound to the exact successor bytes.

Reviewed finite audit batches may therefore feed separately authorized finite
Lean execution manifests, together with explicitly closed support dependencies,
without waiting for unrelated chapters. An audit manifest establishes source
partition and a local denominator; a Lean execution manifest controls proof and
implementation work. Neither substitutes for the other.
...
Before any new DLMF theorem card may freeze, an exact-bound, versioned
successor to `review/classifications-v3.json` must receive independent schema
and source-classification approval. Version 3 and every existing card remain
unchanged; this is a contract for the later reviewed migration, not a schema
implementation in this proposal. Its packet coverage is total and uses
distinct roles:

- `ordinary_source_target`, with an explicit `direct_source_target` or
  `named_source_target` mapping for each generic or named exact/structural
  occurrence;
- `semantic_finite_quantitative`, the existing mandatory finite-producer role
  for each genuinely asymptotic or approximative occurrence; and
- `faithful_source_recovery`, with an explicit target-role mapping for the
  exact printed claim whether that claim is finite or qualitative.

No `mixed` catchall is valid. An entity-identification or notation-equivalence
role cannot substitute for any source-target mapping or earn theorem credit.
Every occurrence in a defined `Target(S,B)` **has** exactly one immutable
source-semantics classification effective in `B`'s pinned classification view:
...
Every `ordinary_exact_or_structural` occurrence has exactly one designated
`ordinary_source_target` mapping, with subtype `direct_source_target` or
`named_source_target`, naming its occurrence, bounded card, declaration, and
eventual accepted exact-bound evidence. Helpers and entity-identification rows
do not replace it. The declaration/evidence terminal states here, like those in
the quantitative mappings below, are completion obligations rather than census
preconditions.
...
cards may instead name proposed Lean objects, nonbinding signature sketches, or
frozen **source-facing mathematical target statements**, with identification,
signature, and implementation obligations explicitly pending. They may not
freeze an exact Lean signature before proof review. New mathematics requires
its complete natural-language proof and all required multiple fresh proof,
quantitative, and architecture/structural reviews before exact Lean signature
design and freezing. The final exact packet then requires external
`lean_ready: pass` before implementation. This sequencing removes no existing
gate.

## 6. Cards, support closure, and stable state

Each Stage 1 target occurrence maps to at least one bounded Stage 1 target card.
For each finite Lean execution manifest `M`, its support set is the **finite
least transitive reviewed dependency closure** rooted at `M`'s selected target
cards and required entity IDs. It follows every mathematical, definitional,
entity/notation-identification, proof, source-recovery, and project-import edge.
The manifest carries a finite membership witness and complete reviewed edge
list; indirect support may not be silently self-declared out of scope. Optional
or speculative cards not reachable from those roots do not enter the closure.

Each support node has exactly one of these kinds:

- an in-range DLMF Chapter 4--10 target atom/card reused by another target;
- source-dependent DLMF Chapters 1--3 support;
- source-dependent DLMF Chapter 11 or later support;
- another exact-bound external-source assertion;
- source-independent LMLF infrastructure; or
- exact pinned-Mathlib interface reuse.

The selected target roots are not counted again as their own support. An in-
range target may nevertheless have a support role for another root, reusing
the same canonical occurrence/card identity; that role earns no additional
occurrence credit, while its independent global target obligation still needs
an accepted target-role mapping in a selected manifest. No source-dependent
mathematics may be relabeled source-independent merely because it is reused.

Every source-dependent node names its exact locator/bytes, statement, consumer,
and reason, receives the same reviewed source classification and explicit
target-role mappings as §5 (including the finite-plus-recovery pair when it is
genuinely asymptotic), and earns zero **additional** Chapter 4--10 coverage in
its support role. An out-of-range source node earns none at all; an in-range
node retains only its independently reconciled target credit. An out-of-range
reference never imports a whole chapter or paper. A Mathlib branch may terminate
at an exact revision and reviewed interface-reuse record; Stage 1 does not
recensus all transitive Mathlib theorems or imports. A dependency review proves
the closure finite and edge-complete and rejects cycles used to launder
requiredness; any legitimate mutually defined finite cluster is one explicit
reviewed support unit with an incoming consumer justification and visible
internal dependencies.

Support is discharged only by exact pinned reuse, a bounded source-independent
card, a bounded exact-source support card, or an honest unresolved/blocked
state. Such a state blocks authorization of a dependent packet whose required
predecessor is unavailable; it does not prevent the support card itself from
advancing through its own natural-language proof, reviews, and `lean_ready`
gate. The finite DAG is implemented topologically without changing its exact
frozen membership. Completion still requires the whole closure terminal.
“Every card,” “orphan card,” and completeness checks in this document
mean only the target cards and the least-closure support records of selected
Stage 1 manifests, never unrelated or unused Olver, BOOTSTRAP-0, IMPROPER-0, or
other project cards.
...
Completion uses the **effective state reconstructed from the selected complete,
exact-bound external authority view**, not literal rewrites of those fields. A
frozen card may therefore
retain `review_status: not_started` and
`implementation_status: not_started` while an external envelope proves later
approval and accepted implementation for exact bytes. R6 never requires
...
| exact source evidence | every source approval used as current authority has retained immutable exact-payload evidence or an authorized byte-guaranteeing archive with the §2 digest, retrieval, availability, access, and rights/provenance bindings; no required audit is `non_reproducible` or `expired`, and a hash without available bytes is insufficient |
| global audit closure | one exact global audit revision `B_all^r` proves that its canonical retrieval-document/leaf membership exhausts Chapters 4--10; `PartitionReady(S,B_all^r)` holds for its pinned source/classification decision view, with complete hierarchy/leaf inventory and semantic atomization exact-bound, exhaustive, nonduplicative, and all document-alias, source-identity-correction, partition, atomicization, role, exclusion, classification, and resolution/lineage reviews approving |
...
| current audit authority | the global closure and every local batch that feeds a required execution manifest have selected, unsuspended exact-bound external authority under §4; the selected authority view includes all applicable accepted adverse, supersession, suspension, and withdrawal records, with no silent fallback, while historical sets remain replayable only as history |
...
| target denominator | `Target(S) = Target(S,B_all^r)` is defined by §4 from atoms effective in the selected closure revision's pinned source view; every member has confirmed applicable notation/entity links and complete card/declaration coverage; older closure revisions retain their exact historical denominators |
...
| classification authority | a reviewed versioned successor classification authority implementing §5 is selected before any new DLMF card freezes; it leaves v3 and old cards unchanged, and no mixed/identification role substitutes for source credit |
| obligation classification | every member of `Target(S)` and every source-dependent support assertion has exactly one immutable §5 classification effective in the selected view, with approving exact-bound external source review; every ordinary target has its reviewed frozen inapplicability reason |
| ordinary mappings | every ordinary-class source assertion has exactly one accepted `ordinary_source_target` mapping of the reviewed direct/named subtype with its exact card, declaration, occurrence, and evidence bindings |
...
| entity/notation authority | every applicable canonical entity has the complete §4 specification, an accepted Lean object, and a proved reviewed identification; aliases/normalizations are resolved, definitions are noncircular, and each audit and execution manifest pins the exact approved notation/entity/association view |
...
| manifests | every required Stage 1 target/support card belongs to at least one finite execution manifest having both `scope_closed: true` and `manifest_status: complete`; every required card's effective `manifest_membership_status` is `complete`, and every required manifest revision/digest is bound into the completion authority |
| dependencies | each execution manifest's finite least transitive support closure has an approving completeness review and explicit witness; zero blocked, unresolved, missing, cyclically laundered, or silently assumed dependencies remain, and all out-of-range/external-source nodes have successful explicit discharge |
| coherent integration | one external root completion/adjudication record binds one exact integration head, Lean toolchain and Mathlib revisions, the selected global closure, every required finite execution-manifest revision/digest, and one complete pinned external authority view; all semantic declarations and recovery consumers coexist and build there with exact source/occurrence joins and passing import, axiom, and regression checks; a candidate-owned assembled report has no selection authority |

The effective state is reconstructed only from the selected complete external
authority view and immutable candidate artifacts it exactly binds. That view is
an explicit decision fold, not a latest-timestamp query; it cannot omit an
accepted adverse record, and material counterevidence invalidates every affected
dependent authorization. Historical passes from mutually incompatible heads,
pins, manifests, source views, or entity views cannot be aggregated into
completion. Existing `review/PROTOCOL.md` invalidation remains strict: an
accepted integration review must name the exact new integration head, and old
verdicts do not form that head's quorum. Earlier investigation may be reused,
but verdict transfer and an invented equivalence waiver are forbidden; this
does not require minting a new historical candidate packet for every unrelated
documentation merge. Compilation, a closed scope, a present envelope, a
defined denominator, or a rendered dashboard proves only itself.
...
| the completion report combines source, proof, implementation, or coverage passes from incompatible historical heads, pins, manifests, or decision views | all accepted products must coexist and pass at one exact integration head and one complete authority view |
| an integration review names an older candidate head, or completion treats an old verdict as quorum for a new integration head | strict exact-head invalidation permits reused investigation, not transferred verdicts |
| the selected authority fold omits an accepted adverse/suspension/withdrawal record because a newer favorable record exists | selection is explicit and complete, never a latest-timestamp filter |
| accepted material counterevidence against one local batch leaves that batch's dependent execution gate or current coverage authoritative | the same root record must suspend affected dependents; unaffected batches alone may continue |
| new DLMF cards freeze before the reviewed successor classification authority, use a `mixed` role, omit explicit target-role mappings, or claim theorem credit through entity identification | packet classification is total and source credit is role-specific |
| a required indirect support edge is omitted, the support witness is infinite/open-ended, or a dependency cycle is used as its own consumer justification | the least transitive support closure must be finite, reviewed, edge-complete, and rooted in selected targets/entities |
| an unused optional card is forced into a manifest solely because it exists | only cards reachable in the selected manifest's least support closure are required |
| a Chapter 4--10 target reused by another card is duplicated as a new support occurrence, gains extra coverage, or loses its independent target-role obligation | one canonical atom/card may have target and support roles, but source counts remain deduplicated and the target mapping remains mandatory |
| an unresolved predecessor is used to authorize its dependent packet, or is treated as forbidding work on its own support card | dependency execution follows the frozen finite DAG topologically through each card's own gates |
| an out-of-range or external-source support node lacks exact bytes, consumer justification, or asymptotic classification, or earns Chapter 4--10 credit | support is exact-bound and classified but never denominator credit |
| pinned Mathlib reuse triggers a demand to recensus all Mathlib imports, or names no exact revision/interface | Mathlib closure terminates only at a reviewed exact pinned-interface reuse record |
| a notation/entity/association repair rebinds an ID in place, coalesces a value-changing normalization as an alias, or omits predecessor/successor lineage | typed identity corrections are append-only and distinguish equality from conversion |
| an unresolved entity mapping removes a reviewed source atom from `Target(S,B)`, or conversely authorizes its named execution | source partition survives identity uncertainty, but named semantic authorization waits for the reviewed entity view |
| an entity repair revokes kernel validity or unrelated batches, or leaves affected identification/card/coverage/recovery authority current | suspension is precise: affected semantic authority pauses, immutable source and kernel facts do not change |
| a renewed source audit relies on a digest while exact payload evidence is unavailable, expired, or lacks rights/provenance | historical decision replay is not rerunnable source evidence or current reproducible approval |
| annotated and unannotated renderings mint duplicate atoms, an independently mathematical annotation is omitted, or a superseded erratum is treated as a competing current truth | annotation payload and history must be reconciled under the selected snapshot state |
| planning freezes an exact Lean signature before the complete natural-language proof and required fresh proof/architecture reviews | only source-facing mathematics may freeze pre-proof; exact signature freezing follows those reviews |
| the discovery checklist omits landing pages, special notation, `Defines`/symbol annotations, or the reverse official index/notation pass but claims its lead map complete | named-family discovery is separately reviewed and explicitly nonexhaustive until all passes close |
...
on the former; if `FX-O2` is already a printed finite bound, `FX.recover` is its
source-notation specialization rather than a fabricated qualitative theorem.
Let `FX-O1` be a direct analyticity assertion and `FX-O4` a named exact
identity; each has one `ordinary_source_target` packet role, an explicit direct
or named target-role mapping, and its approved ordinary classification. The
other ordinary atoms likewise have individual frozen reasons. These are
schema-design examples only, not audited DLMF totals, production rows, or
readiness evidence.

As a positive identity fixture, an unresolved proposed entity link for
`FX-O4` leaves the reviewed five-member `Target(FX-S,FX-B^1)` intact but blocks
the named card and execution manifest that depend on it. A reviewed append-only
successor link can authorize that work without changing the source denominator.
As a positive annotation sentinel, a current attached note that independently
asserts `FX-O6` must receive an atom, while a `Defines` annotation used only to
identify `FX-O6` is shared context and does not create `FX-O7`. As a positive
support fixture, a target card depending on one exact Chapter 2 lemma and one
pinned Mathlib interface has precisely those reviewed reachable nodes in its
finite closure; an unused optional card is absent.

## 9. Audit and review protocol

1. Create the reviewed multi-track schema migration, DLMF source-manifest
   format, and versioned successor classification authority without modifying
   frozen Olver rows, v3 classifications, or historical meanings. The
   representation may vary, but it may not weaken §§2, 4, and 5's canonical-
   document, annotation, alias, partition, ownership, typed source/entity
   lineage, pinned-view, target-role, or total-classification semantics. No new
   DLMF card freezes before that classification authority is approved.
2. Create a finite, explicit Chapter 4 source-audit batch `B` and retrieve
   ...
   per-document dates, digests, redirect chains, version evidence, and the
   retained exact-payload evidence/rights/availability binding required by §2.
   ...
   percentage.
3. Close the separately reviewed discovery-lead pass over landing and special-
   notation pages, every `Defines`/symbol annotation, and reverse official
   index/notation traversal. Keep its list nonexhaustive until closure and do
   not treat it as an atomic denominator. Use
   [§4.13](https://dlmf.nist.gov/4.13) as the omission-sensitive pilot. It
   ...
   names.
4. Extract atomic statements, attached annotations, shared context, local notation, entities,
   ...
   target atom, and pin the append-only notation/entity/association view used by
   each dependent packet.
5. Select only finite closed target/support batches from a reviewed local
   `Target(S,B)`, with a witnessed finite least transitive support closure.
   Record their Lean execution
   manifests separately from the source-audit manifest; neither manifest may
   authorize open-ended chapter membership. New mathematics needs a complete
   natural-language proof and at least two fresh independent pre-Lean reviews—
   source/semantics and proof/quantitative—before exact Lean signature design
   or freezing.
...
6. After external `lean_ready: pass` on the final exact signature packet,
   implement only that packet,
   then obtain build/regression/import/axiom and implementation reviews and
   reconcile coverage. Integration acceptance then binds one exact head, all
   required manifests and decision views, and the Lean/Mathlib pins; all
   declarations and recovery consumers must coexist and pass there. A material
   change invalidates affected approvals under the existing strict protocol.
...
notation, entity, association, card, finite support closure, typed correction/
lineage, exact-source-evidence availability, selected external-authority view,
...
or repair a local authority, omit adverse evidence, choose a mutable “latest”
decision view, or turn a reference lead into coverage.
...
batch, retained exact-payload audit archive, closed discovery-lead pass,
reviewed source partition, defined local or global production
...
global/local selection, complete external authority view, integrated exact
head, external gate, closed/complete DLMF execution manifest, Lean
implementation, or coverage percentage is claimed.

DD-021 revision 5 and this seven-file successor remain pending until the exact
