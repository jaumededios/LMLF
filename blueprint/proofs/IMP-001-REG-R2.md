...
artifact_id: NLP-IMP-001-REG-R2
...
supersedes: NLP-IMP-001-REG-R1
superseded_artifact_binding:
  path: blueprint/proofs/IMP-001-REG.md
  artifact_id: NLP-IMP-001-REG-R1
  sha256: c10dcbc498d80b98f24473a367546963c0b9d6713d99e73e21fd834ff8eedd4c
---
...
This revision is the intrinsically complete frozen successor of the
natural-language analytic support requested by the
`later_mathematical_regression_authorization` clause
...
review. It supersedes frozen R1 solely to correct the exact Mathlib
change-of-variables API binding and record its local side conditions. It
inherits no approval and claims no authorization to write the new regressions
in Lean. At least two fresh independent reviewers must rederive this revision
before any regression whose new mathematics is supplied here is written in
Lean.
...
| `REG-M02` | `intervalIntegral.integral_comp_mul_deriv'` | same import | justify the reciprocal, translation, and reflection substitutions using derivative continuity on `uIcc` and integrand continuity only on the image of the unoriented endpoint interval |
...
Every compact change of variables below uses the prime theorem in `REG-M02`.
For a substitution map \(\phi\), derivative function \(\phi'\), outer
integrand \(g_{\mathrm{out}}\), and endpoints \(a,b\), its three premises are
checked in their exact local form:

\[
 \begin{gathered}
  \forall x\in\operatorname{uIcc}(a,b),\
       \operatorname{HasDerivAt}\phi\,(\phi'(x))\,x,\\
  \operatorname{ContinuousOn}\phi'\bigl(\operatorname{uIcc}(a,b)\bigr),
  \qquad
  \operatorname{ContinuousOn}g_{\mathrm{out}}
       \bigl(\phi[\operatorname{uIcc}(a,b)]\bigr).
 \end{gathered}
 \tag{REG.19a}
\]

Here brackets denote direct image. No global continuity of
\(g_{\mathrm{out}}\) is asserted or needed. For concise total notation, let
\(r=q_0\), let \(r_{\mathrm{abs}}(x)=|q_0(x)|\), and define on all reals

\[
 G(u)=
 \begin{cases}(1-u^{-2})\sin u/u,&u>0,\\0,&u\le0,\end{cases}
 \qquad
 G_{\mathrm{abs}}(u)=
 \begin{cases}(1-u^{-2})|\sin u|/u,&u>0,\\0,&u\le0.\end{cases}
 \tag{REG.19b}
...
Fix \(0<\delta<1\). For the translation
\(\phi_+(t)=t-c\) on endpoints \(a=c+\delta\), \(b=c+1\),
\(\phi_+'(t)=1\) at every point of
\(\operatorname{uIcc}(a,b)=[c+\delta,c+1]\), and the constant derivative is
continuous there. The endpoint images are
\(\phi_+(a)=\delta\), \(\phi_+(b)=1\), and the full interval image is exactly
\([\delta,1]\). Both \(r\) and \(r_{\mathrm{abs}}\) are continuous on this
image: \(\delta>0\) excludes their only singular formula, while absolute value
preserves continuity. Moreover, \(x>0\) and \(1-x^2\ge0\) on the image, so
\[
 r_{\mathrm{abs}}(x)
   =(1-x^2)\frac{|\sin(1/x)|}{x}\qquad(\delta\le x\le1).
\]
Thus REG.19a applies to the original integrand and its absolute value and
gives the translation to \([\delta,1]\).

For the reciprocal step, use \(\phi_{\mathrm{inv}}(x)=1/x\) with the
oriented endpoints \(a=1\), \(b=\delta\). Its unoriented interval is
\(\operatorname{uIcc}(1,\delta)=[\delta,1]\), so
\[
 \operatorname{HasDerivAt}\phi_{\mathrm{inv}}\,(-x^{-2})\,x
 \quad\text{for every }x\in[\delta,1],
\]
and \(x\mapsto-x^{-2}\) is continuous on that interval. The endpoint images
are \(\phi_{\mathrm{inv}}(1)=1\) and
\(\phi_{\mathrm{inv}}(\delta)=1/\delta\), and the full image is exactly
\([1,1/\delta]\). The functions \(G\) and \(G_{\mathrm{abs}}\) are continuous
on this image because it is contained in \((0,\infty)\). On
\([\delta,1]\), direct algebra gives

\[
 (G\circ\phi_{\mathrm{inv}})(x)(-x^{-2})=-r(x),\qquad
 (G_{\mathrm{abs}}\circ\phi_{\mathrm{inv}})(x)(-x^{-2})
    =-r_{\mathrm{abs}}(x).
 \tag{REG.19c}
\]

The orientation \(1\to\delta\) changes
\(\int_1^\delta-r\) into \(\int_\delta^1r\), and similarly for
\(r_{\mathrm{abs}}\). Consequently the translation followed by the
reciprocal substitution gives
...
For the left side, use the reflection \(\phi_-(t)=c-t\) on endpoints
\(a=c-1\), \(b=c-\delta\). At every point of
\(\operatorname{uIcc}(a,b)=[c-1,c-\delta]\) it has derivative
\(\phi_-'(t)=-1\), and this constant derivative is continuous. Its endpoint
images are \(\phi_-(a)=1\), \(\phi_-(b)=\delta\), and its full image is
\([\delta,1]\), where both \(r\) and \(r_{\mathrm{abs}}\) are continuous as
above. Evenness gives \(q_c(t)=r(c-t)\) on this interval. Applying REG.19a
and reversing the \(1\to\delta\) orientation therefore gives the identical
left-side formula
...
Applying the same translation, reflection, and reciprocal calculations with
\(r_{\mathrm{abs}}\) and \(G_{\mathrm{abs}}\) yields exactly the
absolute-value expression in REG.18, not merely an inequality. The endpoint
images remain respectively \((\delta,1)\), \((1,\delta)\), and
\((1,1/\delta)\); the `ContinuousOn` sets are the corresponding closed
unoriented images \([\delta,1]\), \([\delta,1]\), and
\([1,1/\delta]\). Thus every compact substitution is discharged before a
limit is taken, including the absolute-value substitutions.
...
On every compact exhaustion interval
\([k+\alpha,k+\beta]\), \(0<\alpha\le\beta\), this translation has derivative
one everywhere, its derivative is continuous on the `uIcc`, its endpoint
images are \(\alpha,\beta\), and the Gamma integrand and its absolute value
are continuous on the image \([\alpha,\beta]\). Hence REG-M02 also verifies
the compact translated identities. Lebesgue translation invariance then
identifies the restricted open-ray integrals in REG.21; the endpoint
...
no inherited external approval or analytic Lean authorization is claimed.
Reviewers must check in particular the signs
