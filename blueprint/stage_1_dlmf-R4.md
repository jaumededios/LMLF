# Proposed Stage 1: DLMF through Chapter 10 — revision 4
...
**Decision proposed here:** `DD-021`, revision 3
...
This is the complete revision-4 successor proposal to
[`blueprint/stage_1_dlmf-R3.md`](stage_1_dlmf-R3.md). Revisions 1--3 remain
immutable historical review inputs. R3's exact bytes received one approving
review and one `request_changes` review identifying bounded specification
defects; neither verdict is a claim of a current production emergency. No
predecessor has operative authority. This document authorizes no source census,
schema migration, theorem card, Lean implementation, coverage claim, or Verso
authority.

## DD-021 revision 3 — proposed Stage 1 programme precedence
...
This **operative plan is still pending**. DD-021 revision 3 takes effect only
...
hold, the existing frozen control-plane text remains operative and this R4 is
...
DD-021 becomes effective. This scopes scheduling language; it does not alter
historical status evidence or imply acceptance of IMP or any other artifact.
...
historical row. Two typed append-only relations have different effects.

A **within-snapshot correction/resolution** relation selects an effective
atomization for one snapshot and one canonical source appearance. Its split or
merge endpoints must all have that same `snapshot_id` and
`source_appearance_id`. A split names a nonempty successor set and proves that
successors' semantic claim ownership is pairwise nonduplicative and exactly
exhausts the predecessor. A merge proves that its successor owns exactly the
union of its named predecessors, without loss or duplication. Shared context
references remain permitted, and the correction proof must retain or explicitly
map their exact hypotheses and provenance. A zero-successor correction is forbidden unless
an exact-bound reviewed exclusion accounts for the whole former payload; it
can never silently resolve a substantive claim. No within-snapshot merge may
collapse genuinely distinct source appearances.

No correction rebinds or recycles an immutable occurrence ID or ownership key.
Every new successor atom receives a fresh, never-used `atom_ordinal`, ownership
key, and occurrence ID within that same appearance; predecessor identities and
evidence remain intact. The audit revision's selected view determines which
immutable records are effective, never what those records identify.
...
correction, resolution, lineage, fidelity/exclusion, and obligation-
classification decision view used for its result; it never performs a mutable
“latest record” lookup. Its membership is explicit:
...
alias, source-partition, atomicization, fidelity-role, exclusion, effective-
resolution/lineage, and classification reviews must be present and approving
for the pinned view, with no unresolved or invalid lineage inside the batch.
Missing, pending, `changes_requested`, or failed review makes it false.
...
reproducible. Reviewed finite audit batches may nevertheless
...
| global audit closure | one exact global audit revision `B_all^r` is explicitly selected and proves that its canonical retrieval-document/leaf membership exhausts Chapters 4--10; `PartitionReady(S,B_all^r)` holds for its pinned decision view, with complete hierarchy/leaf inventory and semantic atomization exact-bound, exhaustive, nonduplicative, and all document-alias, partition, atomicization, role, exclusion, classification, and resolution/lineage reviews approving |
...
| a within-snapshot split/merge has a semantic gap, duplicate/overlapping claim ownership, an unjustified zero-successor result, or endpoints from different canonical appearances | correction lineage does not preserve one appearance's semantic payload and ownership key |
...
| a within-snapshot merge attempts to collapse otherwise valid atoms at two distinct source appearances | correction merges are appearance-local |
...
2. Create a finite, explicit Chapter 4 source-audit batch `B` and retrieve
   ...
   document identity/aliases, source partitions, semantic atomization, fidelity
   roles, exclusions, classifications, and resolutions. Publish counts only as
...
otherwise undefined denominator, select a mutable “latest” decision view, or
turn a reference lead into coverage.
...
DD-021 revision 3 and this six-file successor remain pending until the exact
