...
The v2-required `QL-001` novelty value is retained even though the bounded-primitive argument is classical. Correcting that mismatch requires an explicit versioned v3 schema and registry migration; this inventory must not mutate frozen v2. `IMP-001` now has an unregistered revision-3 draft card and complete revised author proof for the ordinary-improper/finite-exceptional primitive layer. The endpoint-continuity and component-bookkeeping review findings are integrated; two fresh proof approvals must precede exact-signature design. It remains absent from `cards.csv` until its name, boundary, signatures, and review gates are reconciled.
