...
Card theorem, coverage, and novelty classes are not duplicated in the inventory validator. Their executable allowed sets come directly from current [`review/classifications-v3.json`](../review/classifications-v3.json) packet-level axes; immutable historical artifacts retain their older recorded authority. The validator derives its registry set from each example's `registry_binding`: every `required` row (`QB-001`, `DEF-001`, `QL-001`, `OLV-001`, and `SR-001` currently) must exist in `cards.csv` with all three axes equal, while an `illustrative` row such as `NAMED-EXAMPLE` must not create a registry requirement. Registry status is `planning_only`, `execution_ready`, `active`, `complete`, or `superseded`; artifact status is `planned` or `file_present`.

The v3-required `QL-001` novelty value is now `non_novel`, matching the classical bounded-primitive argument; frozen v2 remains unchanged, and the versioned correction claims no approval. `IMP-001` now has an unregistered revision-6 author draft for the ordinary-improper/finite-exceptional primitive layer. Its eight-target public architecture is justified by the complete QL/OLV consumer route, not by a classification-schema cap: T01 retains independent endpoint semantics, T08 supplies the existential convergent-primitive equivalence, T07 supplies the arbitrary-exception absolute Bochner bridge, T03 permits continuous linear maps `E -> H`, and T06 keeps its finite-piece engine private. IMP R6 and QL R4 correct only three exact pinned root namespaces, and OLV R4 only rebinds those dependencies; the mathematics, architecture, QL a.e. local premise, finite-dimensional source adapter, eight targets, and three modules are unchanged. New exact-commit checks are pending and imply no approval or `lean_ready` status. The revision-3 signature artifact remains immutable superseded evidence. Recent R4 report bytes are absent, and conversation-reported approval creates no current quorum. No R6/R4 proof/architecture review, exact signature or card review, registry/manifest authorization, or Lean authorization exists. IMP-001 remains absent from `cards.csv`.
