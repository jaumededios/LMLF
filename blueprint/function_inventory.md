...
The v3-required `QL-001` novelty value is now `non_novel`, matching the classical bounded-primitive argument; frozen v2 remains unchanged, and the versioned correction claims no approval. `IMP-001` now has an unregistered revision-5 author draft for the ordinary-improper/finite-exceptional primitive layer. Its eight-target public architecture is justified by the complete QL/OLV consumer route, not by a classification-schema cap: T01 retains independent endpoint semantics, T08 supplies the existential convergent-primitive equivalence, T07 supplies the arbitrary-exception absolute Bochner bridge, T03 permits continuous linear maps `E -> H`, and T06 keeps its finite-piece engine private. The revision-3 signature artifact remains immutable superseded evidence. Recent R4 report bytes are absent, and conversation-reported approval creates no current quorum. No R5/R3 proof/architecture review, exact signature or card review, registry/manifest authorization, or Lean authorization exists. IMP-001 remains absent from `cards.csv`.
